...
import {
  ...
  FormControl, InputLabel, Select, Avatar, Tooltip, Paper, Tabs, Tab
} from '@mui/material';
import {
  Add, Search, FilterList, AccessTime, Person,
  PriorityHigh, Edit, Delete, DragIndicator
...
const Tasks = () => {
  ...
  const handleDelete = async () => {
    if (deleteConfirm) {
      try {
        const token = localStorage.getItem('token');
        await axios.delete(`http://localhost:5000/api/tasks/${deleteConfirm}`, {
          headers: { Authorization: `Bearer ${token}` }
        });
